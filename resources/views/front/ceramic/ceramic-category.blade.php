...
<div class="sidebar">
  <section class="bg-white hero-4 no-pg relative head" style="">
    <div class="hero-4__bg" style="background-image: url();">
      <img src="{{$cat->image}}" style="height: 500px;">
    </div>
    <div class="hero-4__content" style="margin-bottom: 198px;background: rgba(255, 0, 0, 0.73);">
      <h1 class="h1 hero-4__content__title" style="color: white">{{$cat->name}}</h1>
    </div>
  </section>  
</div>
...
<div class="sidebar" style="margin-top: -180px;">
  <div class="row">
    <div class="col-sm-4 col-md-3">
      ...
      <div class="">
        <div class="acc">
          <div class="a" style="font-size: 20px">Filtrer la Recherche</div>
        </div>
        <ul id="accordion" class="accordion">
            <li>
                
                <div class="link">Céramique<i class="fa fa-plus-circle"></i></div>
                <ul class="submenu">
                    @isset($categories)
                        <div class="link" style="font-size: 20px">Matiere</div>
                      ...
                      @isset($attributes)
                          ...
                          @if($attribute->name == 'effect' || $attribute->name == 'Ambiance')
                           
                          <div class="link" style="font-size: 20px">{{$attribute->name}}</div>
                      ...
                      @isset($brands)
                        <div class="link" style="font-size: 20px">Marques</div>
            ...
            <li>
                <div class="link">Mosaïque<i class="fa fa-plus-circle"></i></div>
                <ul class="submenu">
                        @isset($attributes)
                          @foreach($attributes as $attribute)
                            @if($attribute->name == 'Designe' || $attribute->name == 'couleur')
                              <div class="link" style="font-size: 20px">{{$attribute->name}}</div>
                              @isset($attribute->options)
                                 @foreach($attribute->options as $option)
                                  <li>
            ...
            <li>
                <div class="link">Salle De Bain<i class="fa fa-plus-circle"></i></div>
                  <ul class="submenu">
                        @isset($attributes)
                          @foreach($attributes as $attribute)
                            @if($attribute->name == 'salle de bain')
                              @isset($attribute->options)
                                 @foreach($attribute->options as $option)
                                  <li>
                                    <div class="checkbox">
...
<script type="text/javascript">
  ...
  function fetchCauseAgainstoptions(opt_id) {
    ...
    $.ajax({
        type: 'GET',
        url: '/category/get_ceramic_by_options/' +opt_id,
        success: function (response) {
            var response = JSON.parse(response);
            console.log(response);
